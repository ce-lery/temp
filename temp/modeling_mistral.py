...
class MistralAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_mask = attention_mask.expand(bsz, self.num_heads, q_len, kv_seq_len).reshape(bsz * self.num_heads, q_len, kv_seq_len)
            attn_weights = attn_weights + attention_mask

        # upcast attention to fp32
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        attn_output = torch.bmm(attn_weights, value_states).view(bsz, self.num_heads, q_len, self.head_dim)
